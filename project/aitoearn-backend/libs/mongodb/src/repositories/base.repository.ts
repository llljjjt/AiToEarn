...
export class BaseRepository<TDocument> {
   ...
   * 根据ID获取单个文档 (别名)
   */
  async findById(id: string, options?: QueryOptions<TDocument>): Promise<LeanDoc<TDocument> | null> {
  ...
  protected async findWithPagination(params: PaginationParams<TDocument>): Promise<readonly [LeanDoc<TDocument>[], number]> {
    const { page, pageSize, filter = {}, options = {}, projection } = params
    const skip = (page - 1) * pageSize

    const findOptions = { ...options, skip, limit: pageSize }

    const [items, total] = await Promise.all([
      this.model.find(filter, projection, findOptions).lean({ virtuals: true }).exec() as Promise<LeanDoc<TDocument>[]>,
